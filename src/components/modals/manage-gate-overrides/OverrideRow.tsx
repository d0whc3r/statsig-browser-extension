...
export const OverrideRow = memo(({ item, canEdit, isPending, onDeleteOverride }: OverrideRowProps) => {
  ...
  return (
    <TableRow className={item.isCurrentUser ? 'bg-muted/30' : undefined}>
      ...
      <TableCell className="text-xs text-muted-foreground">{item.environment ?? 'All Environments'}</TableCell>
      <TableCell>
        <Badge variant={item.type === 'pass' ? 'default' : 'destructive'}>{item.type.toUpperCase()}</Badge>
      </TableCell>
      {canEdit && (
        <TableCell>
          <Button
